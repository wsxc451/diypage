<template>
  <div class="wrap">
    <div class="left">
      <div class="drag-wrap" v-for="item in lists" :key="item.sort" :id="`wrap_${item.sort}`" :ref="`wrap_${item.sort}`">
        <div
          class="drag-item"
  ...
  methods: {
    ...
    changeData(changeList) {
      ...
      this.lists.forEach((wrap, index) => {
        ...
        if (info) {
          this.$set(this.lists, index, Object.assign({}, this.lists[index], { sort: info.sort }))
        }
      })
